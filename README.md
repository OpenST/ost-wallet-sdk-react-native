...
```OstWalletSdk.setupDevice(user.user_id, AppData.TOKEN_ID, new OstWalletWorkflowCallback());```
...
```OstWalletSdk.activateUser(user.user_id,pin,user.user_pin_salt,86400,'1000000000000000000', new ActivateUserCallback());```
...
```OstWalletSdk.addSession(user.user_id, Number(expiryInDays) * 86400, (new BigNumber(spendingLimit)).times((new BigNumber(10)).pow(new BigNumber(18))).toString(), new OstWalletWorkflowCallback());```
...
```OstWalletSdk.executeTransaction(user.user_id, addressesList, amountsList, ruleName, {"type":"user_to_user","name":"Sent to amulya1","details":"Received from preshita"}, new OstWalletWorkflowCallback()); ```
...
```OstWalletSdk.getDeviceMnemonics(user.user_id, new DeviceMnemonicsCallbackImplementation());```
...
```OstWalletSdk.authorizeCurrentDeviceWithMnemonics(user.user_id, passphrase, new OstWalletWorkflowCallback());```
...
```OstWalletSdk.getAddDeviceQRCode(user.user_id, successCallback);```
...
```OstWalletSdk.performQRAction(user.user_id, data, new OstWalletWorkflowCallback());```
...
```OstWalletSdk.resetPin(user.user_id, user.user_pin_salt, currentPin, newPin, new OstWalletWorkflowCallback());```
...
```OstWalletSdk.initiateDeviceRecovery( user.user_id, pin, user.user_pin_salt, deviceAddr, new OstWalletWorkflowCallback());```
...
``` OstWalletSdk.abortDeviceRecovery( user.user_id, pin, user.user_pin_salt, new OstWalletWorkflowCallback());```
...
```OstWalletSdk.logoutAllSessions(user.user_id, new OstWalletWorkflowCallback());```

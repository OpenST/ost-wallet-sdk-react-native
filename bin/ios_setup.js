/*
 ...
 A node script to help set up FBSDK frameworks for use with https://github.com/ostdotcom/ost-wallet-sdk-react-native
 after you have already run `npm install ost-wallet-sdk-react-native`.

 Usage:

 1. add to the 'script' section in package.json of your react-native project the following:
    '"ios:setup": "node ./node_modules/ost-wallet-sdk-react-native/bin/ios_setup.js"'.
...
const zipFileName = 'iOS-Build.zip';
const zipFilePath = path.join(frameworkDir, zipFileName);
const frameworkUrl = 'https://s3.amazonaws.com/akshayraje/iOS-Build.zip';
...
function correctSearchPaths (next) {
    const rctfbsdkProjectPath = './node_modules/ost-wallet-sdk-react-native/ios/OstWalletRNSdk.xcodeproj/project.pbxproj';
...
function updateFBSDKFrameworks (next) {
    ...
    myProj.parse(function (err) {
        ...
        myProj.addFramework('./ios/Frameworks/OstWalletSdk.framework', { 'customFramework': true, 'target': target, 'link': true });
...
fall([downloadFbSdk, unzipFramework, correctSearchPaths, updateFBSDKFrameworks], done);
